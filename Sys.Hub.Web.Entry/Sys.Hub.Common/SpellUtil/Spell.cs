...
namespace Sys.Hub.Common.Util
{
    /// <summary>
    /// 创 建 人 ：  胖太乙
    /// 创建时间 ：  2023/5/4 20:37:52 
    /// 描    述 ：  汉字转拼音
    /// </summary>
    public static class Spell
    {
        ...
        public static string GetChineseText(string pinyin)
        {
            ...
            foreach (string str in SpellCode.codes)
            {
        ...
        public static string GetPinyin(char ch)
        {
            ...
            for (var i = 0; i < SpellHash.hashes[hash].Length; ++i)
            {
                short index = SpellHash.hashes[hash][i];
                var pos = SpellCode.codes[index].IndexOf(ch, 7);
                if (pos != -1)
                    return SpellCode.codes[index].Substring(0, 6).Trim();
            }
            return ch.ToString();
        }
        ...
        {
            return (short)((uint)ch % SpellCode.codes.Length);
        }

        /// <summary>
        /// 拼音首写字母排序
        /// </summary>
        /// <param name="ListArray">首写字母集合</param>
        /// <returns></returns>
        public static SortedSet<string> SpellInitialsSort(List<string> ListArray)
        {
            SortedSet<string> list = new SortedSet<string>(ListArray);
            return list;
        }
    }
}
